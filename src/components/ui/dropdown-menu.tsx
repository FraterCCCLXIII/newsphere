...
function isScrollInsideRadixMenuContent(target: EventTarget | null): boolean {
  if (!(target instanceof Node)) return false;
  const el = target instanceof Element ? target : target.parentElement;
  if (!el) return false;
  return el.closest("[data-radix-menu-content]") !== null;
}
...
>(({ className, sideOffset = 4, ...props }, ref) => {
  React.useEffect(() => {
    const onScroll = (e: Event) => {
      if (isScrollInsideRadixMenuContent(e.target)) return;
